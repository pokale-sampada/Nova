...
# current_agent_login_logout=agent_get("agent_three","http://action_login_server:5008/webhook")
# current_agent_leave_mgmt=agent_get("agent_four","http://action_leavemgmt_server:5009/webhook")
# current_agent_profile_info=agent_get("agent_five","http://action_profile_server:5010/webhook")
# current_agent_faq=agent_get("agent_six","http://action_faq_server:5012/webhook")
# current_agent_leave_approval=agent_get("agent_seven","http://action_leaveapp_server:5013/webhook")
...
def new_message():  
  ...
  confidences = [confidence_leave_mgmt,confidence_profile_info,confidence_leave_approval]

  #allskill = ["Training_Skill","TTE Skill","Leave Management","Profile Info","FAQ Skill","Leave Approval"]
  allskill = ["Leave Management","Profile Info","Leave Approval"]
  ...
  if current_model == "Leave_Management":
    ...
    if state == "cancel_switch":
      ...
      # Check if the user input has meaning or confidence is below a certain threshold
      confidence_profile_info, intent_name_profile_info = loop.run_until_complete(confidence(current_agent_profile_info, user_message))
      print("profile info confidence", confidence_profile_info)
  
      # Handle random input or low confidence
      if confidence_profile_info < 0.7 or intent_name_profile_info == "random":
          message = [{
              'recipient_id': user,
              'text': "I couldn't understand that. Please provide valid input.",
              'buttons': [{'title': 'Home', 'payload': 'home'}]
          }]  
      elif state == "cancel_switch":
          state = None
  ...
  #     message = message_faq
    ...
    print(intent_name_login_logout,"intent_name_login_logout")
    print(len(user_message),"6666666666666")
    ...
    try:
      ...
      for x in message_login_logout:    
        ...
        if x['text'].find("employee code is") != -1:
          ...
          message_leave_approval = loop.run_until_complete(process(current_agent_leave_approval,"hi",user))
          sent_msg.append(x)
