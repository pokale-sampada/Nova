...
class ActionCheckARFR(Action):
    ...
    def run(self, dispatcher, tracker, domain):
        if Status == "AR" or Status == "FR":
            print("yessif....")
            dispatcher.utter_template('utter_help_user_after_loggedin1', tracker)
        else:
            print("yesselse")
            dispatcher.utter_template("utter_help_user1", tracker)
        return []
...
class ActionGetListOfPendingApproval(Action):
    ...
    def run(self, dispatcher, tracker, domain):
        ...
        try:
            ...
            data = response.json()
            print("data action_get_list_of_pending_approval", data)
            print("len", len(data))
            ...
            try:
                ...
                for particular_leave in range(0, leaves_count):
                    ...
                    other_emp_code_dict.update(
                        {data[particular_leave]['Emp_code']: data[particular_leave]['lr_id']})
                    print(other_emp_code_dict,"!!!!!!!!!!!!!!!222")
                    gt.append({
                        "type": "List",
                        "level": "third level",
                        "lr_id": data[particular_leave]['lr_id'],
                        "emp1_code": data[particular_leave]['Emp_code'],
                        "title": "<b>Pending Leave Requests:</b> Please review the following leave requests that require your approval. To take action, click on the respective request.",
                ...
                print("User_asked_before_intent get a list of pending leave approval", User_asked_before_intent)
                confirmation_box_slot = ['ordinal', 'command']
            except:
                if data['errorDesc'] == "No applications exists!":
                    dispatcher.utter_message("There is no leave requests pending with me for approval.")
        ...
        # print(confirmation_box)
        print("245")
        return [SlotSet('command', None), SlotSet('empCodeForleaveRequest', None), SlotSet('reason', None),
                SlotSet('ordinal', None), SlotSet('name2', None), SlotSet('emp_code1', None)]
...
class ConfirmationBoxForm(FormValidationAction):
            ...
            if sr_no in confirmation_box.keys() or sr_no in other_emp_code_dict.keys():
                ...
                try:
                    ...
                    buttons.append({"title": "Reject",
                                    "payload": "reject"})
                    print(
                        "---------------------------------------------sssssssss",confirmation_box[num][1],confirmation_box[num][6])
                    ...
                    buttons.append({"title": "Reject",
                                    "payload": "reject"})
                    print(confirmation_box_emp_code[num],"except loop")
                    dispatcher.utter_button_message(
                        "<b>Employee code: </b>{}<br><b>Name:</b> {}<br><b>Type of leave:</b> {}<br><b>From:</b> {}<br><b>To:</b> {}<br><b>Number of days:</b> {}<br><b>Reason of leave:</b> {}".format(
    ...
    def validate_command(
        ...
        if value.lower() in ["approve","reject"]:
...
class EmpCodeCollectionForm(FormValidationAction):
    ...
    async def required_slots(
        ...
        action_check = tracker.get_slot('command')
        print(tracker.get_slot('command'))
        global emp_code_collect_form_slots
        try:
            if action_check.lower() == 'approve':
                print("Inside IF OF required slot approve")
                return ['command', 'empCodeForleaveRequest']
            elif action_check.lower() == 'reject':
                print("Inside IF OF required slot reject")
                return ['command', 'empCodeForleaveRequest']
            else:
                print("inside else required slots")
        ...
        if len(emp_code_collect_form_slots) > 0:
            ...
            try:
                ...
                if empCodeForleaveRequest in other_emp_code_dict.keys():
                    ...
                    if tracker.get_slot('command') == "reject":
                        ...
                        print("reject second hit", response)
                        data1 = response.json()
                        print(data1)
                        print(User_asked_before_intent)
                        if data1['errorCode'] is 0:
                            dispatcher.utter_message("You have rejected this leave successfully!!")
                            buttons.append({"title": "Yes",
                            ...
                            "{}/approveRejectLeaveAPI?emp_code={}&emp1_code={}&lr_id={}&action={}&comment={}".format(
                                mindsconnect_url, tracker.get_slot('emp_code'), empCodeForleaveRequest, lr_id,
                                tracker.get_slot('command'), tracker.get_slot('reason')))
                            print("errorcode second hit", response2)
                        ...
                        if data1['errorCode'] is 0:
                            ...
                            "{}/approveRejectLeaveAPI?emp_code={}&emp1_code={}&lr_id={}&action={}&comment={}".format(
                                mindsconnect_url, tracker.get_slot('emp_code'), empCodeForleaveRequest, lr_id,
                                tracker.get_slot('command'), tracker.get_slot('reason')))
                            print("error code 0second hit", response2)
                            dispatcher.utter_message("You have approved this leave successfully!!")
                            buttons = []
                            buttons.append({"title": "Yes",
                                    "payload": "leave requests"})
                            buttons.append({"title": "No",
                            ...
                            "{}/approveRejectLeaveAPI?emp_code={}&emp1_code={}&lr_id={}&action={}&comment={}".format(
                                mindsconnect_url, tracker.get_slot('emp_code'), empCodeForleaveRequest, lr_id,
                                tracker.get_slot('command'), tracker.get_slot('reason')))
                            print("error code 32second hit", response2)
                            data2 = response2.json()
                            print(data2)
                            if data2['errorCode'] is 0:
                                dispatcher.utter_message("You have approved this leave successfully!!")
    ...
    def run(self, dispatcher, tracker, domain):
        ...
        try:
            ...
            try:
                ...
                global User_asked_before_intent
                User_asked_before_intent = tracker.latest_message['intent'].get('name')
                print(User_asked_before_intent,"user asked befosandjakndkankdnak")
                confirmation_box_slot = ['ordinal', 'command']
            except:
                if data['errorDesc'] == "No applications exists!":
