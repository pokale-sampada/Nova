...
mindsconnect_url = "https://demo.omfysgroup.com/mindsconnectleapi" #FORUATDEMO
# mindsconnect_url ="https://mindsconnect.omfysgroup.com/" #FORPRODUCTION
...
#   else:
    ...
    max_match_indFex = confidence_percent.index(max(confidence_percent)) 
...
class UserLoginForm(FormValidationAction):
    ...
    def validate_emp_code(self, value: Text, dispatcher: CollectingDispatcher, tracker: Tracker,
        ...
        if data != None:
            ...
            if value == "home":
                dispatcher.utter_message("Please first sign up")
            else:
                dispatcher.utter_message("Could you please provide me correct Employee Id")
        ...
        if data != None:
            try:
                if data["errorCode"]==208 and data['errorMsg'] == "Failed." :
                    ...
                    dispatcher.utter_message("Wrong Password")
...
class ActionUserLogin(Action):
    ...
    def run(self, dispatcher, tracker, domain):
        ...
        try:
            ...
            print(f"emp_code type: {type(emp_code)}, emp_code value: {emp_code}")
            print(f"password type: {type(password)}, password value: {password}")
            print(f"data type: {type(data)}, data content: {data}")
            
            if 'errorMsg' not in data or data['errorMsg'] is None:
                # If 'errorMsg' is not present or is None in data dictionary
                ...
                return []
            elif data['errorMsg']=="Invalid username !! Please enter valid username.":
                dispatcher.utter_message("Could you please provide me correct Employee Id")
            else:
                dispatcher.utter_template("utter_ask_password", tracker)
                return []
